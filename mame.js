javascript:(function() {
    ...
    // NEW: Remove the specific SVG icon
    const removeSvgIcon = () => {
        // Find all SVG elements on the page
        const svgs = document.querySelectorAll('svg');
        // A unique fill color from one of the paths in the target SVG
        const uniqueFillColor = "#DE5833"; 
        
        svgs.forEach(svg => {
            // Check if this SVG contains a path with the unique fill color
            const path = svg.querySelector(`path[fill="${uniqueFillColor}"]`);
            if (path) {
                // If it's the target SVG, remove it
                svg.remove();
            }
        });
    };
    ...
    removeElements();
    replaceImages();
    removeSvgIcon(); // <-- Run the new function
    ...
    const observer = new MutationObserver(() => {
        ...
        replaceImages();
        replaceText(document.body);
        removeSvgIcon(); // <-- Run the new function on changes
    });
